...
export async function POST(request: NextRequest) {
  try {
    ...
    // For extension requests, skip NextAuth session check and use userId from request
    // The extension stores userId in chrome.storage.local
    if (!userId) {
      return NextResponse.json({ error: 'userId is required' }, { status: 400 })
    }

    // Check rate limit using userId
...
continue
      ...
      // Derive rewardType from currency_type
